...
async function drawPosts() {
    ...
    for (let post of data) {
        ...
        for (let comm of commentData) {
            ...
            buttonDel.classList.add(`comm-del_button`)
...
drawPosts().then(async () => {
    let buttonArr = document.querySelectorAll(`.PostButton`)
    let inputArr = document.querySelectorAll(`.PostInput`)
    let buttonDelArr = document.querySelectorAll(`.comm-del_button`)
    for (let but of buttonArr) {
        but.addEventListener(`click`, () => {
            for (let inp of inputArr) {
                if (but.getAttribute(`idPostButton`) == inp.getAttribute(`idPostInput`)) {
                    makeQuery(`comment/create`, `POST`, {
                        message: `${inp.value}`,
                        owner: localStorage.getItem(`authorizedUser`),
                        post: but.getAttribute(`idPost`),
                    })
                }
            }
        })
    }
    for (let but of buttonDelArr) {
        but.addEventListener(`click`, () => {
            let id = but.getAttribute(`idComment`)
            makeQuery(`comment/${id}`, `DEL`)
        })
    }
})
async function drawCustomPost() {
    ...
    for (let post of data) {
        ...
        postP.style.color = `red`
        postDiv.appendChild(postP)
        let commentData = await makeQuery(`post/${post.id}/comment?created=1`)
        for (let comm of commentData) {
            let p = document.createElement(`p`)
            let h5 = document.createElement(`h5`)
            let buttonDel = document.createElement(`button`)
